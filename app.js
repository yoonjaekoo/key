...
function loadKeys(){try{const saved=JSON.parse(localStorage.getItem(STORAGE_KEY));if(Array.isArray(saved)&&saved.length===DEFAULT_KEYS.length&&saved.every(k=>k&&typeof k.code==='string'&&typeof k.lbl==='string'))return saved}catch(e){}return DEFAULT_KEYS.map(k=>({...k}))}
let KEYS=loadKeys();
const N=KEYS.length,ORANGE='#ff6a00',CAP_H=48,PAD=6,MAX_PARTICLES=5;
let speedMult=3.0,dirUp=true,totalHits=0,maxKps=0,listeningIndex=null;
const lanes=Array.from({length:N},()=>[]),particles=Array.from({length:N},()=>[]),pulseAt=new Float64Array(N),hitBuffer=[],pressedCodes=new Set();
...
const codeMap={};
function rebuildCodeMap(){for(const k in codeMap)delete codeMap[k];KEYS.forEach((k,i)=>codeMap[k.code]=i);const hi=KEYS.findIndex(k=>k.lbl==='한/영'||k.code==='Lang1');if(hi!==-1)['HangulMode','KanaMode','Convert','NonConvert','Lang1'].forEach(code=>codeMap[code]=hi)}
rebuildCodeMap();
function prettyKey(e){const special={Space:'␠',Tab:'⇥',Backspace:'⌫',Enter:'↵',Escape:'Esc',ArrowUp:'↑',ArrowDown:'↓',ArrowLeft:'←',ArrowRight:'→',Equal:'=',Minus:'-',Comma:',',Period:'.',Slash:'/',Backslash:'\\',Semicolon:';',Quote:"'",BracketLeft:'[',BracketRight:']',Backquote:'`',CapsLock:'Caps',ShiftLeft:'LShift',ShiftRight:'RShift',ControlLeft:'LCtrl',ControlRight:'RCtrl',AltLeft:'LAlt',AltRight:'RAlt',Lang1:'한/영',HangulMode:'한/영'};if(special[e.code])return special[e.code];if(e.code.startsWith('Key'))return e.code.slice(3);if(e.code.startsWith('Digit'))return e.code.slice(5);if(e.code.startsWith('Numpad'))return 'N'+e.code.slice(6);return e.key&&e.key.length<=8?e.key:e.code}
function registerDown(code,repeat=false){const i=codeMap[code];if(i===undefined||repeat||pressedCodes.has(code))return;pressedCodes.add(code);pressLane(i)}
function registerUp(code){const i=codeMap[code];if(i===undefined)return;pressedCodes.delete(code);closeSegment(i)}
function handleKeyDown(e){if(listeningIndex!==null){e.preventDefault();e.stopPropagation();if(e.repeat)return;const target=listeningIndex,other=KEYS.findIndex((k,i)=>i!==target&&k.code===e.code);if(other!==-1){const old={...KEYS[target]};KEYS[other]=old}KEYS[target]={code:e.code,lbl:prettyKey(e)};listeningIndex=null;localStorage.setItem(STORAGE_KEY,JSON.stringify(KEYS));rebuildCodeMap();renderKeyGrid();resize();return}if(codeMap[e.code]===undefined)return;e.preventDefault();e.stopPropagation();registerDown(e.code,e.repeat)}
function handleKeyUp(e){if(listeningIndex!==null||codeMap[e.code]===undefined)return;e.preventDefault();e.stopPropagation();registerUp(e.code)}
window.addEventListener('keydown',handleKeyDown,true);window.addEventListener('keyup',handleKeyUp,true);
if(globalThis.chrome?.runtime?.onMessage){chrome.runtime.onMessage.addListener(message=>{if(!message||message.type!=='ADOFAI_KEY_VIEWER_EVENT'||listeningIndex!==null)return;if(message.kind==='keydown')registerDown(message.code,Boolean(message.repeat));else if(message.kind==='keyup')registerUp(message.code)})}
